...
import type { ExportArgs, SlideInfo, TocItem } from '@kolibrijs/types'
...
function createKolibriProgress(indeterminate = false) {
...
export async function exportNotes({
  ...
  if (!packageExists('playwright-chromium'))
    throw new Error('The exporting for Kolibri is powered by Playwright, please install it via `npm i -D playwright-chromium`')
  ...
  const progress = createKolibriProgress(true)
...
export async function exportSlides({
  ...
  const page = await context.newPage()
  const progress = createKolibriProgress(!perSlide)
...
export function getExportOptions(args: ExportArgs, options: ResolvedKolibriOptions, outDir?: string, outFilename?: string): Omit<ExportOptions, 'port' | 'base'> {
  const config = {
