...
function copyCode() {
  const code = el.value?.querySelector('.kolibry-code')?.textContent
  if (code)
    copy(code)
}
</script>
...
<template>
  ...
  >
    ...
    <button
      ...
      class="kolibry-code-copy absolute top-0 right-0 transition opacity-0 group-hover:opacity-20 hover:!opacity-100"
