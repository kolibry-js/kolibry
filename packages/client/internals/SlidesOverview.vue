...
<template>
  ...
  >
    ...
    >
      ...
      >
        ...
        >
          <div
            class="inline-block border rounded border-opacity-50 overflow-hidden bg-main hover:border-$kolibry-theme-primary transition"
            :class="(focus(idx + 1) || currentOverviewPage === idx + 1) ? 'border-$kolibry-theme-primary' : 'border-gray-400'"
            :style="themeVars"
            @click="go(+route.path)"
          >
            <SlideContainer
  ...
  <button v-if="value" class="fixed text-2xl top-4 right-4 kolibry-icon-btn text-gray-400" @click="close">
