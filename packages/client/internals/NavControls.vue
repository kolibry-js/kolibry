...
import CustomNavControls from '/@enigmaslidev/custom-nav-controls'
...
<template>
  <nav ref="root" class="flex flex-col">
    ...
    >
      <button v-if="!isEmbedded" class="enigmaslidev-icon-btn" @click="toggleFullscreen">
      ...
      <button class="enigmaslidev-icon-btn" :class="{ disabled: !hasPrev }" @click="prev">
        <HiddenText text="Go to previous slide" />
        <carbon:arrow-left />
      </button>

      <button class="enigmaslidev-icon-btn" :class="{ disabled: !hasNext }" title="Next" @click="next">
        <HiddenText text="Go to next slide" />
        <carbon:arrow-right />
      </button>

      <button v-if="!isEmbedded" class="enigmaslidev-icon-btn" title="Slides overview" @click="toggleOverview()">
      ...
      <template v-if="__ENIGMASLIDEV_FEATURE_DRAWINGS__ && (!configs.drawings.presenterOnly || isPresenter) && !isEmbedded">
        <button class="enigmaslidev-icon-btn relative" title="Drawing" @click="drawingEnabled = !drawingEnabled">
      ...
      <template v-if="!isEmbedded">
        ...
        <RouterLink v-if="__ENIGMASLIDEV_FEATURE_PRESENTER__ && showPresenter" :to="presenterLink" class="slidev-icon-btn" title="Presenter Mode">
          <carbon:user-speaker />
        </RouterLink>

        <button
          v-if="__DEV__ && __ENIGMASLIDEV_FEATURE_EDITOR__"
          class="enigmaslidev-icon-btn <md:hidden"
        ...
        <button v-if="isPresenter" class="enigmaslidev-icon-btn" title="Toggle Presenter Layout" @click="togglePresenterLayout">
          <carbon:template />
          {{ presenterLayout }}
        </button>
      </template>
      <template v-if="!__DEV__">
        <button v-if="configs.download" class="enigmaslidev-icon-btn" @click="downloadPDF">
      ...
      <template v-if="!isPresenter && !isEmbedded">
        <MenuButton>
          <template #button>
            <button class="enigmaslidev-icon-btn">
