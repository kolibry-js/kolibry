...
const fontSize = useLocalStorage('kolibry-notes-font-size', 18)
  ...
  <div class="h-full flex flex-col">
    ...
    <div class="flex-none border-t border-gray-400 border-opacity-20">
      <div class="flex gap-1 items-center px-6 py-3">
        <button class="kolibry-icon-btn" @click="toggleFullscreen">
          <HiddenText :text="isFullscreen ? 'Close fullscreen' : 'Enter fullscreen'" />
          <carbon:minimize v-if="isFullscreen" />
          <carbon:maximize v-else />
        </button>
        <button class="kolibry-icon-btn" @click="increaseFontSize">
          <HiddenText text="Increase font size" />
          <carbon:zoom-in />
        </button>
        <button class="kolibry-icon-btn" @click="decreaseFontSize">
