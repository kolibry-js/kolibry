...
export function createMonacoTypesLoader(): Plugin {
  return {
    name: 'kolibry:monaco-types-loader',

    resolveId(id) {
      if (id.startsWith('/@kolibry-monaco-types/'))
        return id
      return null
    },

    async load(id) {
      const match = id.match(/^\/\@kolibry-monaco-types\/(.*)$/)
      if (match) {
        const pkg = match[1]
        ...
        return [
          ...
          ...Object.keys(pkgJson.dependencies || {}).map(i => `import "/@kolibry-monaco-types/${i}"`),
