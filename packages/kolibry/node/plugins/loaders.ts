...
import { isString, notNullish, objectMap, range, slash, uniq } from '@nyxb/utils'
...
import type { KolibryMarkdown, SlideInfo, SlideInfoExtended } from '@kolibry/types'
import * as parser from '@kolibry/parser/fs'
import equal from 'fast-deep-equal'

import type { LoadResult } from 'rollup'
import type { KolibryPluginOptions, KolibryServerOptions, ResolvedKolibryOptions } from '../options'
import { resolveImportPath, stringifyMarkdownTokens, toAtFS } from '../utils'

const regexId = /^\/\@kolibry\/slide\/(\d+)\.(md|json)(?:\?import)?$/
...
const vueContextImports = [
  ...
  `import {
    injectionKolibryContext as _injectionKolibryContext, 
  ...
  } from "@kolibry/client/constants.ts"`.replace(/\n\s+/g, '\n'),
  'const $kolibry = _vueInject(_injectionKolibryContext)',
  'const $nav = _vueToRef($kolibry, "nav")',
...
export function createSlidesLoader(
  { data, entry, clientRoot, themeRoots, addonRoots, userRoot, roots, remote, mode }: ResolvedKolibryOptions,
  pluginOptions: KolibryPluginOptions,
  serverOptions: KolibryServerOptions,
): Plugin[] {
  const slidePrefix = '/@kolibry/slides/'
  ...
  return [
    {
      ...
      async handleHotUpdate(ctx) {
        ...
        if (hmrPages.size > 0)
          moduleIds.add('/@kolibry/titles.md')
      ...
      resolveId(id) {
        if (id.startsWith(slidePrefix) || id.startsWith('/@kolibry/'))
          return id
        return null
      },
      ...
      load(id): LoadResult | Promise<LoadResult> {
        // routes
        if (id === '/@kolibry/routes')
          return generateRoutes()

        // layouts
        if (id === '/@kolibry/layouts')
          return generateLayouts()

        // styles
        if (id === '/@kolibry/styles')
          return generateUserStyles()

        // monaco-types
        if (id === '/@kolibry/monaco-types')
        ...
        // global component
        if (id === '/@kolibry/global-components/top')
          return generateGlobalComponents('top')

        // global component
        if (id === '/@kolibry/global-components/bottom')
          return generateGlobalComponents('bottom')

        // custom nav controls
        if (id === '/@kolibry/custom-nav-controls')
          return generateCustomNavControls()

        // title
        if (id === '/@kolibry/titles.md') {
          return {
            code: data.slides
    ...
    {
      name: 'kolibry:layout-transform:pre',
      enforce: 'pre',
    ...
    {
      name: 'kolibry:context-transform:pre',
      enforce: 'pre',
      async transform(code, id) {
        if (!id.endsWith('.vue') || id.includes('/@kolibry/client/') || id.includes('/packages/client/'))
          return
        return transformVue(code)
      },
    },
    {
      name: 'kolibry:title-transform:pre',
      enforce: 'pre',
      transform(code, id) {
        if (id !== '/@kolibry/titles.md')
          return
    ...
    {
      name: 'kolibry:slide-transform:post',
      enforce: 'post',
      transform(code, id) {
        if (!id.match(/\/@kolibry\/slides\/\d+\.md($|\?)/))
          return
  ...
  async function transformMarkdown(code: string, pageNo: number, data: KolibryMarkdown) {
    ...
    const imports = [
      ...
      ';(() => {',
      '  const route = $kolibry.nav.rawRoutes.find(i => i.path === String($page.value))',
  ...
  function transformVue(code: string): string {
    if (code.includes('injectionKolibryContext') || code.includes('injectionClicks') || code.includes('const $kolibry'))
    ...
    if (matchScript && matchScript[2]) {
      ...
      if (matchExport) {
        ...
        const provideImport = '\nimport { injectionKolibryContext } from "@kolibry/client/constants.ts"\n'
        code = `${code.slice(0, scriptIndex)}${provideImport}${code.slice(scriptIndex)}`

        let injectIndex = exportIndex + provideImport.length
        let injectObject = '$kolibry: { from: injectionKolibryContext },'
...
defineProps<{ no: number | string }>()`)
  ...
  async function generateMonacoTypes() {
    return `void 0; ${parser.scanMonacoModules(data.raw).map(i => `import('/@kolibry-monaco-types/${i}')`).join('\n')}`
  }
