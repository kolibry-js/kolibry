...
import type { ResolvedShikiOptions, ShikiOptions } from '@kolibry/types'
  ...
  markdownit.options.highlight = (code, lang) => {
    if (darkModeThemes) {
      ...
      const dark = _highlighter
        ...
        .replace('<pre class="shiki', '<pre class="kolibry-code shiki shiki-dark')
      const light = _highlighter
        .codeToHtml(trimmed, { lang: lang || 'text', theme: darkModeThemes.light })
        .replace('<pre class="shiki', '<pre class="kolibry-code shiki shiki-light')
      ...
      return escapeVueInCode(
        _highlighter.codeToHtml(code, { lang: lang || 'text' })
          .replace('<pre class="shiki"', '<pre class="kolibry-code shiki"'),
      )
    }
  }
}
