...
export async function exportNotes({
  ...
  if (!packageExists('playwright-chromium'))
    throw new Error('The exporting for Kolibry is powered by Playwright, please install it via `npm i -D playwright-chromium`')
  ...
  const progress = createKolibryProgress(true)
...
export async function exportSlides({
  ...
  const page = await context.newPage()
  const progress = createKolibryProgress(!perSlide)
...
export function getExportOptions(args: ExportArgs, options: ResolvedKolibryOptions, outDir?: string, outFilename?: string): Omit<ExportOptions, 'port' | 'base'> {
  const config = {
