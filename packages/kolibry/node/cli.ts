...
import type { KolibryConfig, KolibryPreparserExtension } from '@kolibry/types'
import isInstalledGlobally from 'is-installed-globally'
import equal from 'fast-deep-equal'
import { verifyConfig } from '@kolibry/parser'
import { injectPreparserExtensionLoader } from '@kolibry/parser/fs'
...
const CONFIG_RESTART_FIELDS: (keyof KolibryConfig)[] = [
  'highlighter',
  'monaco',
  'routerMode',
  'fonts',
...
injectPreparserExtensionLoader(async (headmatter?: Record<string, unknown>, filepath?: string) => {
  ...
  const mergeArrays = (a: KolibryPreparserExtension[], b: KolibryPreparserExtension[]) => a.concat(b)
...
cli.command(
  ...
  args => commonOptions(args)
    ...
    .option('tunnel', {
      default: false,
      type: 'boolean',
      describe: 'open localtunnel to make Kolibry available on the internet',
    })
    .option('log', {
      default: 'warn',
      type: 'string',
